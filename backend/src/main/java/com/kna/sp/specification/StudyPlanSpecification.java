...
public final class StudyPlanSpecification {
    ...
    public static Specification<StudyPlan> withFilters(
        ...
        return Specification.<StudyPlan>allOf(
                year == null ? Specification.unrestricted()
                        : (root, query, cb) -> cb.equal(root.get("year"), year),
                month == null ? Specification.unrestricted()
                        : (root, query, cb) -> cb.equal(root.get("month"), month),
                fromDate == null ? Specification.unrestricted() : (root, query, cb) ->
                        cb.greaterThanOrEqualTo(root.join("sessions", JoinType.LEFT).get("studyDate"), fromDate),
                toDate == null ? Specification.unrestricted() : (root, query, cb) ->
                        cb.lessThanOrEqualTo(root.join("sessions", JoinType.LEFT).get("studyDate"), toDate),
                subjectId == null ? Specification.unrestricted() : (root, query, cb) ->
